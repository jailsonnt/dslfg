...
public class Subtracao implements Funcao {
	private Objeto resultado;
	private Object itemDaDireita;
	private Object itemDaEsquerda;

	public Subtracao(Object itemDaEsquerda, Object itemDaDireita) {
	...
	public void executar(ContextoDeExecucao contextoDeExecução) throws ExcecaoEmTempoDeExecucao {
		...
		resultado = executaSubtração(itemDaEsquerda, itemDaDireita);
	...
	private Objeto executaSubtração(Objeto itemDaEsquerda, Objeto itemDaDireita){
		if (verificaSeRetornoDeveraSerTexto(itemDaEsquerda, itemDaDireita)){
			return new Texto(itemDaEsquerda.toString().replace(itemDaDireita.toString(), ""));
		}else{
