...
public class ContextoDeExecucaoSe extends ContextoDeExecucaoAbstrato implements Executavel {
	private Funcao condição;

	public ContextoDeExecucaoSe(ArrayList<Executavel> executáveis, Funcao condição) {
	...
	public void executar(ContextoDeExecucao contextoDeExecução) throws ExcecaoEmTempoDeExecucao {
		...
		if (condição.obterResultado().equals(new Booleano(true))){
	...
	public Executavel obterCópia() {
		...
		return new ContextoDeExecucaoSe(executáveis, condição);
	}
